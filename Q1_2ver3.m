...
solxyz= solve([eq11, eq12, eq13 ], [x,y,z]); % two set of fixed point (y,z)
fp1q1= [solxyz.x(1,1) solxyz.y(1,1) solxyz.z(1,1)]; % depend on f
fp2q1= [solxyz.x(2,1) solxyz.y(2,1) solxyz.z(2,1)]; % depend on f
fp3q1= [solxyz.x(3,1) solxyz.y(3,1) solxyz.z(3,1)]; % (0,0)
fm= (0.1:0.1:2); % condition 1
for i= 1:numel(fm)
    f= fm(i);
    fp1q1f(i,1)= double(subs(solxyz.x(1,1))); % table fp 1 from question 4
    fp1q1f(i,2)= double(subs(solxyz.y(1,1))); 
    fp1q1f(i,3)= double(subs(solxyz.z(1,1)));
    fp2q1f(i,1)= double(subs(solxyz.x(2,1))); % table fp 2 from question 4
    fp2q1f(i,2)= double(subs(solxyz.y(2,1))); 
    fp2q1f(i,3)= double(subs(solxyz.z(2,1)));
    fp3q1f(i,1)= double(subs(solxyz.x(3,1))); % table fp 3 from question 4
    fp3q1f(i,2)= double(subs(solxyz.y(3,1)));
    fp3q1f(i,3)= double(subs(solxyz.z(3,1)));
end
